          ...
          <TableBody>
            {filtered.map((i) => (
              <TableRow key={i.id}>
                ...
                <TableCell>
                  <Badge className={
                    i.severity === "Critical" ? "bg-destructive text-destructive-foreground" :
                    i.severity === "High" ? "bg-orange-500 text-white" :
                    i.severity === "Medium" ? "bg-amber-500 text-white" :
                    "bg-emerald-500 text-white"
                  }>
                    {i.severity}
                  </Badge>
                </TableCell>
                <TableCell className="hidden lg:table-cell">{i.state}</TableCell>
                <TableCell className="hidden xl:table-cell">{i.assigned_to || "—"}</TableCell>
                <TableCell className="hidden xl:table-cell">{Math.round(i.confidenceScore * 100)}%</TableCell>
                <TableCell>
                  <div className="flex items-center gap-2">
                    <span className={`h-2 w-2 rounded-full ${i.escalationProbability > 0.7 ? "bg-destructive" : i.escalationProbability > 0.45 ? "bg-orange-500" : "bg-emerald-500"}`}></span>
